package com.gowrimanikandan.thiranx.features.task.status;
...
class TaskStatusUpdateModel {
    ...
    void updateStatus(Task task, Task.TaskStatus newStatus, String remarks, Employee changedBy) {
        ...
        if (newStatus == Task.TaskStatus.COMPLETED) {
            ...
            updatePerformanceMetrics(task, changedBy);
    ...
    private void updatePerformanceMetrics(Task task, Employee changedBy) {
        if (task == null || changedBy == null) return;

        // For now, we'll give a default rating of 4.0 for completed tasks
        // In a real application, this could be based on task difficulty, time taken, etc.
        double rating = 4.0;
        ThiranXDB.getInstance().updatePerformance(changedBy.getId(), true, rating);
    }
