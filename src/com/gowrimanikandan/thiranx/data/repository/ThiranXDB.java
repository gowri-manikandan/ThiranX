package com.gowrimanikandan.thiranx.data.repository;
...
import com.gowrimanikandan.thiranx.data.dto.TaskStatusHistory;
...
public class ThiranXDB {
    ...
    public List<Employee> searchEmployees(String query) {
        ...
        if (key.isEmpty())
            return result;
        for (Employee current : employees) {
            if ((current.getEmployeeId() != null && current.getEmployeeId().toLowerCase(Locale.ROOT).contains(key))
                    || (current.getName() != null && current.getName().toLowerCase(Locale.ROOT).contains(key))
                    || (current.getEmail() != null && current.getEmail().toLowerCase(Locale.ROOT).contains(key))) {
                result.add(current);
            }
        }
        return result;
    ...
    public List<Employee> getEmployeesExcept(Long excludeId) {
        List<Employee> result = new ArrayList<>();
        for (Employee current : employees) {
            if (current.getStatus() != Employee.EmployeeStatus.ACTIVE)
                continue;
    ...
    public List<Task> getTasksAssignedTo(Long employeeId) {
        ...
        for (Task current : tasks) {
            if (employeeId.equals(current.getAssignedTo()))
                result.add(current);
        }
        return result;
    }
    ...
    public List<Task> getTasksCreatedBy(Long assignedById) {
        ...
        if (assignedById == null)
            return result;
        for (Task current : tasks) {
            if (assignedById.equals(current.getAssignedBy()))
                result.add(current);
        }
        return result;
    }
    ...
    public List<Employee> getDirectReports(Long managerId) {
        ...
        if (managerId == null)
            return result;
        for (Employee current : employees) {
            if (managerId.equals(current.getReportingTo()))
                result.add(current);
        }
        return result;
    }
    ...
    public Notification addNotification(Notification notification) {
        if (notification == null || notification.getEmployeeId() == null)
            return null;
        notificationPk++;
    ...
    public Performance getOrCreatePerformance(Long employeeId) {
        if (employeeId == null)
            return null;
        for (Performance current : performances) {
            if (employeeId.equals(current.getEmployeeId())) {
                return current;
            }
        }
        ...
        performancePk++;
        performance.setId(performancePk);
        performance.setEmployeeId(employeeId);
        performance.setCompletedTasks(0);
        performance.setTotalTasks(0);
        performance.setAverageRating(0.0);
        performance.setLastUpdated(System.currentTimeMillis());
        performances.add(performance);
        return performance;
    }

    public Performance updatePerformance(Long employeeId, boolean taskCompleted, double rating) {
        Performance performance = getOrCreatePerformance(employeeId);
        if (performance == null)
            return null;

        performance.setTotalTasks(performance.getTotalTasks() + 1);
        if (taskCompleted) {
            performance.setCompletedTasks(performance.getCompletedTasks() + 1);
        }

        // Update average rating
        double currentTotalRating = performance.getAverageRating() * (performance.getTotalTasks() - 1);
        performance.setAverageRating((currentTotalRating + rating) / performance.getTotalTasks());
    ...
    public Performance getPerformanceByEmployeeId(Long employeeId) {
        if (employeeId == null)
            return null;
        for (Performance current : performances) {
            if (employeeId.equals(current.getEmployeeId())) {
